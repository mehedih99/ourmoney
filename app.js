...
async function loadTx(){const {data,error}=await db.from("transactions").select("*").is("deleted_at",null).order("transaction_date",{ascending:false}).order("created_at",{ascending:false});if(error)throw error;state.transactions=data||[]}
...
function txHtml(x){
  ...
  return `<div class="tx"><div class="tx-icon">${x.type==="income"?"＋":x.type==="expense"?"−":"↗"}</div><div class="tx-main"><strong>${esc(label)}</strong><small>${x.transaction_date} • ${tr("addedBy")}: ${esc(x.owner_name||ownerName(x.created_by))}${x.payment_method?` • ${esc(x.payment_method)}`:""}${x.note?` • ${esc(x.note)}`:""}</small></div><div class="tx-amount ${x.type}">${sign} ${money(x.amount,x.currency)}</div><button class="icon-btn tx-menu" data-id="${x.id}">⋯</button></div>`;
}
function bindTxMenu(){$$(".tx-menu").forEach(b=>b.onclick=async()=>{if(!confirm(lang()==="bn"?"রেকর্ডটি ট্র্যাশে পাঠাবেন?":"Move this record to Trash?"))return;await db.from("transactions").update({deleted_at:new Date().toISOString()}).eq("id",b.dataset.id);await loadTx();renderAll()})}
...
function openEntry(type="expense"){setEntryType(type);$("#entryDate").value=localDate();$("#entryAmount").value="";$("#entryNote").value="";$("#entryCurrency").value=state.settings.base_currency;$("#entryRate").value=1;$("#autoOwner").textContent=state.profile.display_name;updateRate();openD("entryDialog")}
...
async function submitEntry(e){
  ...
  const row={type,amount,currency,exchange_rate:rate,transaction_date:$("#entryDate").value,payment_method:$("#entryMethod").value,note:$("#entryNote").value.trim()||null,created_by:state.user.id,owner_name:state.profile.display_name};
...
function pdfReport(){
  ...
  doc.autoTable({startY:y,head:[["Date","Type","Category","Subcategory / Source","Amount","Added by"]],body:rows.map(x=>[x.transaction_date,x.type,x.category||"",x.subcategory||x.source||x.saving_account||"",money(x.amount,x.currency),x.owner_name||ownerName(x.created_by)]),theme:"grid",styles:{fontSize:7}});
  doc.save(`our-money-${state.reportType}-${localDate()}.pdf`);
}

function renderSettings(){
  $("#preferenceTitle").textContent=state.profile?.display_name||"—";$("#profileName").value=state.profile?.display_name||"";$("#profileLanguage").value=lang();
...
async function saveProfile(){const n=$("#profileName").value.trim(),l=$("#profileLanguage").value;if(!n)return;const {error}=await db.from("profiles").update({display_name:n,language:l}).eq("user_id",state.user.id);if(error)return toast(error.message);state.profile={...state.profile,display_name:n,language:l};await loadProfiles();applyLanguage();initMonthPicker();populate();renderAll();$("#sidebarUser").textContent=n;$("#autoOwner").textContent=n}
