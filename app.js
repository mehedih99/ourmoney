...
function populate(){
  ...
  fill($("#reportPayWith"),[{value:"all",label:tr("all")},{value:"Bank",label:"Bank"},{value:"Cash",label:"Cash"},{value:"Credit",label:"Credit"}],"all");fill($("#reportCreditAccount"),[{value:"all",label:"All Credit Accounts"},...state.creditAccounts.map(a=>({value:a.id,label:a.name}))],"all");
...
function renderDashboard(){
  ...
  $("#incomeTotal").textContent=money(inc);$("#expenseTotal").textContent=money(exp);renderSavingsMasked(sav);const ledgerNow=calculateAccountBalances();$("#availableBalance").textContent=ledgerNow.missing?"—":money(ledgerNow.total);$("#totalSavings").textContent=money(allSav);renderHeroAccountBreakdown();prepareExpenseSlides(tx);
...
function renderCreditDashboard(){if(!$("#creditTotalDue"))return;$("#creditTotalDue").textContent=money(state.creditAccounts.reduce((s,a)=>s+creditOutstanding(a.id),0));$("#creditTotalAvailable").textContent=money(state.creditAccounts.filter(a=>a.active!==false).reduce((s,a)=>s+creditAvailable(a.id),0));$("#creditAccountCards").innerHTML=state.creditAccounts.filter(a=>a.active!==false).map(a=>{const used=creditOutstanding(a.id),limit=Number(a.credit_limit||0),pct=limit?Math.min(100,used/limit*100):0;return `<div class="credit-card-row"><div class="credit-card-top"><strong>${esc(a.name)}</strong><span>${a.account_type==="credit_card"?"Credit Card":"Installment Service"}</span></div><div class="credit-card-numbers"><div>Limit<b>${money(limit)}</b></div><div>Used<b>${money(used)}</b></div><div>Available<b>${money(creditAvailable(a.id))}</b></div></div><div class="credit-progress"><span style="width:${pct}%"></span></div></div>`}).join("")||`<div class="empty-state">Add Credit Account in Settings</div>`}
function creditHasHistory(id){return state.transactions.some(x=>x.credit_account_id===id)||state.creditPayments.some(x=>x.credit_account_id===id)}
function renderCreditSettings(){if(!$("#creditManager"))return;$("#creditManager").innerHTML=state.creditAccounts.map(a=>{const used=creditOutstanding(a.id),has=creditHasHistory(a.id),action=has?`<button class="btn credit-archive" data-id="${a.id}">${a.active===false?"Activate":"Archive"}</button>`:`<button class="btn credit-delete" data-id="${a.id}">Delete</button>`;return `<div class="credit-manager-row"><div><strong>${esc(a.name)}</strong><small>${a.account_type==="credit_card"?"Credit Card":"Installment Service"} • Limit ${money(a.credit_limit)} • Used ${money(used)} • Available ${money(creditAvailable(a.id))}</small></div><div class="credit-manager-actions"><button class="btn credit-edit" data-id="${a.id}">Edit</button>${action}</div></div>`}).join("")||`<div class="empty-state">No credit accounts</div>`;$$(".credit-edit").forEach(b=>b.onclick=()=>editCreditAccount(b.dataset.id));$$(".credit-archive").forEach(b=>b.onclick=()=>toggleCreditAccount(b.dataset.id));$$(".credit-delete").forEach(b=>b.onclick=()=>deleteCreditAccount(b.dataset.id))}
async function editCreditAccount(id){if(!navigator.onLine)return toast("Internet required");const a=creditAccount(id);if(!a)return;const name=prompt("Credit account name",a.name);if(name===null||!name.trim())return;const lim=prompt("Credit limit (AED)",String(a.credit_limit||0));if(lim===null)return;const n=Number(lim);if(!Number.isFinite(n)||n<0)return toast("Invalid limit");if(n+.000001<creditOutstanding(id))return toast("Limit cannot be below current used amount");const {error}=await db.from("credit_accounts").update({name:name.trim(),credit_limit:n,updated_at:new Date().toISOString()}).eq("id",id);if(error)return toast(error.message);await loadCreditAccounts();populate();renderAll();toast("Credit account updated")}
async function toggleCreditAccount(id){if(!navigator.onLine)return toast("Internet required");const a=creditAccount(id);if(!a)return;if(a.active!==false&&creditOutstanding(id)>0)return toast("Clear outstanding due before archiving");const {error}=await db.from("credit_accounts").update({active:a.active===false,updated_at:new Date().toISOString()}).eq("id",id);if(error)return toast(error.message);await loadCreditAccounts();populate();renderAll()}
async function deleteCreditAccount(id){if(!navigator.onLine)return toast("Internet required");if(creditHasHistory(id))return toast("This account has history. Archive it instead.");const a=creditAccount(id);if(!a)return;if(!confirm(`Delete ${a.name}?`))return;const {error}=await db.from("credit_accounts").delete().eq("id",id);if(error)return toast(error.message);await loadCreditAccounts();populate();renderAll();toast("Credit account deleted")}
...
function creditPaymentHtml(x){const a=creditAccount(x.credit_account_id),p=state.transactions.find(t=>t.id===x.credit_purchase_id);return `<div class="tx credit-payment-tx" data-credit-payment-id="${x.id}"><div class="tx-icon">▣</div><div class="tx-main"><strong>Credit Payment → ${esc(a?.name||"Credit")}</strong><small>${x.payment_date} • Bank${p?` • ${esc(p.category||"Purchase")}${p.subcategory?` → ${esc(p.subcategory)}`:""}`:""} • ${esc(x.owner_name||ownerName(x.created_by))}</small></div><div class="tx-amount transfer">${money(x.amount,x.currency)}</div><button class="icon-btn credit-payment-menu" data-id="${x.id}">⋯</button></div>`}
...
function bindHistoryItems(){
  ...
  $$(".tx-menu").forEach(b=>b.onclick=async(e)=>{
    ...
    const deletingTx=state.transactions.find(x=>x.id===id);if(deletingTx?.payment_source==="Credit"&&creditPurchasePaid(id)>0)return toast("This credit purchase has payments. Delete/reverse the payments first.");
  ...
  $$(".credit-payment-menu").forEach(b=>b.onclick=async(e)=>{e.stopPropagation();if(!navigator.onLine)return toast("Credit payment changes require internet");if(!confirm("Delete this credit payment? Bank balance and credit due will be recalculated."))return;const {error}=await db.from("credit_payments").update({deleted_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",b.dataset.id);if(error)return toast(error.message);await loadCreditPayments();renderAll();toast("Credit payment deleted")});
...
function updateInstallmentPreview(){const box=$("#installmentPreview");if(!box)return;const show=$("#expensePayWith").value==="Credit"&&$("#expensePlan").value==="installment";box.classList.toggle("hidden",!show);if(!show)return;const amount=Number($("#entryAmount").value||0),n=Number($("#installmentCount").value||0),first=$("#firstDueDate").value;if(amount>0&&n>=2)box.textContent=`${n} installments • approx. ${money(amount/n,$("#entryCurrency").value)} each${first?` • first due ${first}`:""}`;else box.textContent="Enter amount and installment count"}
...
function getReportRows(){
  const {from,to}=reportDateRange(),cat=$("#reportCategory").value||"all",sub=$("#reportSubcategory").value||"all",incomeOwner=$("#reportIncomeOwner").value||"all",payWith=$("#reportPayWith").value||"all",creditFilter=$("#reportCreditAccount").value||"all",owner=$("#reportOwner").value||"all",cur=$("#reportCurrency").value||"all";
  if(state.reportType==="transfer"){let rows=state.transfers.filter(x=>x.transfer_date>=from&&x.transfer_date<=to);if(payWith==="Bank"||payWith==="Cash")rows=rows.filter(x=>x.from_account===payWith||x.to_account===payWith);if(payWith==="Credit")rows=[];if(owner!=="all")rows=rows.filter(x=>(x.owner_name||ownerName(x.created_by))===owner);if(cur!=="all")rows=rows.filter(x=>x.currency===cur);return rows.map(x=>({...x,_kind:"transfer"}))}
  if(state.reportType==="credit"){let rows=state.transactions.filter(x=>x.type==="expense"&&x.payment_source==="Credit"&&x.transaction_date>=from&&x.transaction_date<=to);if(creditFilter!=="all")rows=rows.filter(x=>x.credit_account_id===creditFilter);if(cat!=="all")rows=rows.filter(x=>x.category===cat);if(sub!=="all")rows=rows.filter(x=>x.subcategory===sub);if(owner!=="all")rows=rows.filter(x=>(x.owner_name||ownerName(x.created_by))===owner);if(cur!=="all")rows=rows.filter(x=>x.currency===cur);return rows.map(x=>({...x,_kind:"transaction"}))}
  let rows=state.transactions.filter(x=>x.transaction_date>=from&&x.transaction_date<=to);if(state.reportType==="family")rows=rows.filter(x=>x.type==="expense"&&x.category==="Family Support");else if(state.reportType!=="all")rows=rows.filter(x=>x.type===state.reportType);
  if(incomeOwner!=="all")rows=rows.filter(x=>x.income_owner===incomeOwner);if(payWith!=="all"){if(payWith==="Credit")rows=rows.filter(x=>x.type==="expense"&&x.payment_source==="Credit");else rows=rows.filter(x=>effectiveAccount(x)===payWith)}if(creditFilter!=="all")rows=rows.filter(x=>x.payment_source==="Credit"&&x.credit_account_id===creditFilter);if(cat!=="all")rows=rows.filter(x=>x.category===cat);if(sub!=="all")rows=rows.filter(x=>x.subcategory===sub);if(owner!=="all")rows=rows.filter(x=>(x.owner_name||ownerName(x.created_by))===owner);if(cur!=="all")rows=rows.filter(x=>x.currency===cur);
  if(state.reportType==="all"){const tf=state.transfers.filter(x=>x.transfer_date>=from&&x.transfer_date<=to).filter(x=>(payWith==="all"||((payWith==="Bank"||payWith==="Cash")&&(x.from_account===payWith||x.to_account===payWith)))&&(owner==="all"||(x.owner_name||ownerName(x.created_by))===owner)&&(cur==="all"||x.currency===cur)).map(x=>({...x,_kind:"transfer"}));return [...rows.map(x=>({...x,_kind:"transaction"})),...tf]}return rows.map(x=>({...x,_kind:"transaction"}))
}
function reportLabel(){
  if(state.reportType==="income")return tr("totalIncome");if(state.reportType==="expense")return tr("totalExpense");if(state.reportType==="saving")return tr("totalSavings");if(state.reportType==="family")return tr("totalFamily");if(state.reportType==="transfer")return tr("totalTransfer");if(state.reportType==="credit")return "Total Credit Purchases";return lang()==="bn"?"নেট পরিবর্তন":"Net Change";
}
function renderReport(){
 const rows=getReportRows();state.reportRows=rows;let total=state.reportType==="all"?rows.reduce((s,x)=>x._kind==="transfer"?s:s+(x.type==="income"?baseValue(x):-baseValue(x)),0):state.reportType==="transfer"?rows.reduce((s,x)=>s+Number(x.amount||0)*Number(x.exchange_rate||1),0):rows.reduce((s,x)=>s+baseValue(x),0);const label=reportLabel();$("#reportSummaryLabel").textContent=label;$("#reportSummaryValue").textContent=money(total);$("#reportSummaryMeta").textContent=`${rows.length} ${tr("entries")}`;$("#reportBottomLabel").textContent=label;$("#reportBottomTotal").textContent=money(total);$("#reportCount").textContent=`${rows.length} ${tr("entries")}`;
 const payWith=$("#reportPayWith").value||"all",showCredit=state.reportType==="expense"||state.reportType==="credit"||state.reportType==="all";$("#reportPayWithWrap").classList.toggle("hidden",state.reportType==="income"||state.reportType==="saving"||state.reportType==="family");$("#reportCreditAccountWrap").classList.toggle("hidden",!showCredit||!(payWith==="Credit"||state.reportType==="credit"));$("#creditReportSummary").classList.toggle("hidden",!(state.reportType==="credit"||payWith==="Credit"));
 if(state.reportType==="credit"||payWith==="Credit"){const {from,to}=reportDateRange(),cid=$("#reportCreditAccount").value||"all";let purchases=state.transactions.filter(x=>x.type==="expense"&&x.payment_source==="Credit"&&x.transaction_date>=from&&x.transaction_date<=to),pays=state.creditPayments.filter(p=>p.payment_date>=from&&p.payment_date<=to);if(cid!=="all"){purchases=purchases.filter(x=>x.credit_account_id===cid);pays=pays.filter(x=>x.credit_account_id===cid)}const accounts=cid==="all"?state.creditAccounts:state.creditAccounts.filter(a=>a.id===cid);$("#creditReportPurchases").textContent=money(purchases.reduce((s,x)=>s+baseValue(x),0));$("#creditReportPaid").textContent=money(pays.reduce((s,p)=>s+Number(p.amount||0)*Number(p.exchange_rate||1),0));$("#creditReportDue").textContent=money(accounts.reduce((s,a)=>s+creditOutstanding(a.id),0));$("#creditReportAvailable").textContent=money(accounts.reduce((s,a)=>s+creditAvailable(a.id),0))}
 const cmap={},smap={};if(state.reportType==="income")rows.forEach(x=>{const k=x.income_owner||"Unassigned";cmap[k]=(cmap[k]||0)+baseValue(x);if(x.source)smap[x.source]=(smap[x.source]||0)+baseValue(x)});else if(state.reportType==="transfer")rows.forEach(x=>{const k=`${x.from_account} → ${x.to_account}`;cmap[k]=(cmap[k]||0)+Number(x.amount||0)*Number(x.exchange_rate||1)});else rows.filter(x=>x._kind!=="transfer").forEach(x=>{if(x.category)cmap[x.category]=(cmap[x.category]||0)+baseValue(x);if(x.subcategory)smap[x.subcategory]=(smap[x.subcategory]||0)+baseValue(x)});const breakdown=m=>{const a=Object.entries(m).sort((a,b)=>b[1]-a[1]);return a.length?a.map(([k,v])=>`<div class="breakdown-item"><span>${esc(k)}</span><strong>${money(v)}</strong></div>`).join(""):`<div class="empty-state">${tr("noData")}</div>`};$("#reportCategoryBreakdown").innerHTML=breakdown(cmap);$("#reportSubcategoryBreakdown").innerHTML=breakdown(smap);$("#reportBreakdownWrap").classList.toggle("hidden",state.reportType==="saving");$("#reportIncomeOwnerWrap").classList.toggle("hidden",!(state.reportType==="income"||state.reportType==="all"));const d=x=>x._kind==="transfer"?x.transfer_date:x.transaction_date,t=x=>x._kind==="transfer"?(x.transfer_time||""):(x.transaction_time||"");const sorted=[...rows].sort((a,b)=>`${d(b)} ${t(b)}`.localeCompare(`${d(a)} ${t(a)}`));$("#reportTransactions").innerHTML=sorted.length?sorted.map(historyItemHtml).join(""):`<div class="empty-state">${tr("noData")}</div>`;bindHistoryItems()
}
function pdfReport(){const {jsPDF}=window.jspdf,doc=new jsPDF({unit:"pt",format:"a4"}),rows=state.reportRows;let y=42;doc.setFont("helvetica","bold");doc.setFontSize(18);doc.text("Our Money - Finance Report",40,y);y+=18;doc.setFont("helvetica","normal");doc.setFontSize(9);doc.text(`Generated: ${new Date().toLocaleString()} | User: ${state.profile.display_name}`,40,y);y+=20;let total=state.reportType==="all"?rows.reduce((s,x)=>x._kind==="transfer"?s:s+(x.type==="income"?baseValue(x):-baseValue(x)),0):state.reportType==="transfer"?rows.reduce((s,x)=>s+Number(x.amount||0)*Number(x.exchange_rate||1),0):rows.reduce((s,x)=>s+baseValue(x),0);doc.autoTable({startY:y,head:[[reportLabel(),"Entries"]],body:[[money(total),String(rows.length)]],theme:"grid",styles:{fontSize:9}});y=doc.lastAutoTable.finalY+14;const body=rows.map(x=>{if(x._kind==="transfer")return[x.transfer_date,"transfer",`${x.from_account} > ${x.to_account}`,money(x.amount,x.currency),x.owner_name||ownerName(x.created_by)];let details=x.category?`${x.category}${x.subcategory?` > ${x.subcategory}`:""}`:(x.source||x.saving_account||"");if(x.payment_source==="Credit"){const a=creditAccount(x.credit_account_id);details+=` | Credit: ${a?.name||"Credit"}`;if(Number(x.installment_count||1)>1){const ni=nextInstallmentInfo(x);details+=` | ${x.installment_count} installments${ni?` | Next ${money(ni.amount)} ${ni.date}`:""}`}}else if(effectiveAccount(x))details+=` | ${effectiveAccount(x)}`;return[x.transaction_date,x.type,details,money(x.amount,x.currency),x.owner_name||ownerName(x.created_by)]});doc.autoTable({startY:y,head:[["Date","Type","Details","Amount","Added by"]],body,theme:"grid",styles:{fontSize:7}});doc.save(`our-money-${state.reportType}-${localDate()}.pdf`)}
...
async function addCreditAccount(){if(!navigator.onLine)return toast("Internet required to change Credit Accounts");const name=$("#newCreditName").value.trim(),type=$("#newCreditType").value,limit=Number($("#newCreditLimit").value||0);if(!name)return toast("Enter credit account name");if(state.creditAccounts.some(a=>a.name.toLowerCase()===name.toLowerCase()))return toast("A Credit Account with this name already exists");const {error}=await db.from("credit_accounts").insert({name,account_type:type,credit_limit:limit,created_by:state.user.id});if(error)return toast(error.message);$("#newCreditName").value=$("#newCreditLimit").value="";await loadCreditAccounts();populate();renderAll()}
...
function updateCreditPayPurchases(){const id=$("#creditPayAccount").value,p=creditPurchases(id);fill($("#creditPayPurchase"),p.map(x=>{const ni=nextInstallmentInfo(x);return {value:x.id,label:`${x.category||"Purchase"}${x.subcategory?` → ${x.subcategory}`:""} • Remaining ${money(creditPurchaseRemaining(x))}${ni?` • Next ${ni.date}`:""}`}}));if(!p.length)$("#creditPayHint").textContent="No outstanding purchase for this account";updateCreditPayHint()}
...
function bind(){
  ...
  $("#reportCategory").onchange=()=>{updateReportSubs();renderReport()};["reportSubcategory","reportIncomeOwner","reportPayWith","reportCreditAccount","reportOwner","reportCurrency","reportFrom","reportTo"].forEach(id=>$("#"+id).onchange=renderReport);$("#downloadPdfBtn").onclick=pdfReport;
